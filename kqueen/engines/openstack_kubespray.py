...
class OpenstackKubesprayEngine(BaseEngine):
    ...
    def __init__(self, cluster, *args, **kwargs):
        ...
        self.kwargs = kwargs
        short_uuid = base64.b32encode(uuid.UUID(cluster.id).bytes)[:26]
        self.stack_name = "kqn-" + short_uuid.decode('ascii')
        self.ks = Kubespray(
    ...
    def deprovision(self):
        ...
        try:
            resources = self.cluster.metadata.get("resources")
            if not resources:
                return True, "Cluster has no resources"
            self.os.deprovision(self.cluster.metadata["resources"])
        except Exception as e:
    ...
    def engine_status(cls, **kwargs):
        try:
            ...
            #openstack = OpenStack(os_kwargs=kwargs,
...
class Kubespray:
    ...
    def _run_ansible(self, inventory="hosts.json", playbook="cluster.yml"):
        ...
        args = [config.KS_ANSIBLE_PLAYBOOK_CMD, "-b", "-i",
                inventory, playbook,
    ...
    def get_ssh_key(self):
        ...
        if not os.path.exists(ssh_key_path):
            cmd = [config.KS_SSH_KEYGEN_CMD, "-P", "", "-f", ssh_key]
...
class OpenStack:
    ...
    def __init__(self, cluster_name, *, os_kwargs, cluster, extra_ssh_key):
    ...
    def provision(self):
        ...
        network = self.c.create_network(self.cluster_name)
        subnet = self.c.create_subnet(network, cidr="10.1.0.0/16",
                                      subnet_name=self.cluster_name,
                                      dns_nameservers=dns)
        router = self.c.create_router(name=self.cluster_name,
        ...
        # TODO: security group
        for master in self._boot_servers(name=self.cluster_name + "-master-",
        ...
        # TODO: security group
        name = "-slave-%s-" % base64.b32encode(os.urandom(10)).decode('ascii')
        for slave in self._boot_servers(name=self.cluster_name + name,
    ...
    def deprovision(self, resources):
        for server in resources["masters"] + resources["slaves"]:
            self.c.delete_server(server["id"], wait=True)
        for master in resources["masters"]:
            self.c.delete_floating_ip(master["floating_ip_id"])
        router = self.c.get_router(self.cluster_name)
        ...
        self.c.delete_network(self.cluster_name)
